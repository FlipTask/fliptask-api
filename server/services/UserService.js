...
class UserService extends CrudService {
  ...
  _checkPassword = async (
      candidate, saved) => { return await bcrypt.compare(candidate, saved); }

  _createAuthToken =
      async (user) => {
    const token = crypto.randomBytes(20).toString('hex');
    const expiresAt =
        new Date(new Date().getTime() + this.TOKEN_EXPIRY_MILLISECONDS);

    const authToken = await AuthToken.create({token, expiresAt, user});
